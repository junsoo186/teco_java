...
public class MyBookStore {
	...
	public static void main(String[] args) {
		...
		books[1] = new Book("무궁화꽃이피었습니다", "김진영");
	...
	public static void readAll(Book[] books) {
		...
		for (int i = 0; i < books.length; i++) {
			// 방어적 코드 작성
			if (books[i] != null) {
				System.out.println(" 책 이름 : " + books[i].getTitle() + " / 작가 :" + books[i].getAuthor());
			}
		}
	...
	public static void readByTitle(Scanner sc, Book[] books) {
		...
		// 사용자가 입력한 책 제목
		// books 전부 조사 book[0] --> 객체 --> 객체 . getTitle()
		// 문자열 --> equals
		// books [0] .getTitle().equals() // 배열안에 객테가 없다면 --> null pointer -->방어적 코드
		for (int i = 0; i < books.length; i++) {
			// 방어적 코드 작성
			if (books[i] != null) {
